...
extension MicRecorder: RecordingAudioSource {
    ...
    ///
    /// A phone on the table hears the whole room through one microphone, so there's no "your side"
    /// to separate out the way the Mac app does — every voice, the user's included, lands on this
    /// single stream. `.others` is the stream the transcriber requests speaker labels for, so
    /// mapping the mic to it is what turns the mix into "Speaker A"/"Speaker B" rather than one
