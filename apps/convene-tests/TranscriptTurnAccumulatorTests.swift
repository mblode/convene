...
final class TranscriptTurnAccumulatorTests: XCTestCase {
    ...
    func testPartialReplacesTextThenFinalizes() {
        ...
        XCTAssertNil(
            acc.handleTurn(
                turnMessage(order: 0, endOfTurn: false, transcript: "so the"), speaker: .you,
                currentElapsed: 0))
        ...
        XCTAssertNil(
            acc.handleTurn(
                turnMessage(order: 0, endOfTurn: false, transcript: "so the plan"), speaker: .you,
                currentElapsed: 0))
        XCTAssertEqual(acc.segments.count, 1)
        XCTAssertEqual(acc.segments[0].text, "so the plan")

        let confirmed = acc.handleTurn(
            turnMessage(order: 0, endOfTurn: true, transcript: "So the plan."), speaker: .you,
            currentElapsed: 0)
        XCTAssertEqual(acc.segments.count, 1)
    ...
    func testEmptyFinalDropsAccumulatedPartial() {
        var acc = TranscriptTurnAccumulator()
        _ = acc.handleTurn(
            turnMessage(order: 0, endOfTurn: false, transcript: "half a thought"), speaker: .you,
            currentElapsed: 0)
        XCTAssertEqual(acc.segments.count, 1)

        let confirmed = acc.handleTurn(
            turnMessage(order: 0, endOfTurn: true, transcript: "   "), speaker: .you, currentElapsed: 0)
    ...
    func testEchoDuplicateOnMicIsDroppedKeepingOthersCopy() {
        ...
        let othersConfirmed = acc.handleTurn(
            turnMessage(
                order: 0, endOfTurn: true, transcript: echoPhrase,
    ...
    func testSpeakerRevisionRelabelsKnownTurnAndIgnoresUnknown() {
        var acc = TranscriptTurnAccumulator()
        _ = acc.handleTurn(
            turnMessage(order: 0, endOfTurn: true, transcript: "First.", speakerLabel: "A"), speaker: .others,
            currentElapsed: 0)

        // Turn 0 is known; turn 42 was never seen and must be ignored without crashing.
        acc.applySpeakerRevision(
            AssemblyAISpeakerRevisionMessage(revisions: [0: "B", 42: "C"]), speaker: .others)
        XCTAssertEqual(acc.segments.count, 1)
    ...
    func testResetClearsSegments() {
        var acc = TranscriptTurnAccumulator()
        _ = acc.handleTurn(
            turnMessage(order: 0, endOfTurn: true, transcript: "Something."), speaker: .you, currentElapsed: 0
        )
