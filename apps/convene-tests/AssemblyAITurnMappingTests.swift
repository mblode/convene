...
final class AssemblyAITurnMappingTests: XCTestCase {
    ...
    func testPartialGrowsThenFinalizesByReplacingText() {
        ...
        transcriber.handleServerMessage(
            makeTurn(
                order: 0, endOfTurn: false, transcript: "so the plan",
        ...
        transcriber.handleServerMessage(
            makeTurn(
                order: 0, endOfTurn: false, transcript: "so the plan for this quarter",
        ...
        transcriber.handleServerMessage(
            makeTurn(
                order: 0, endOfTurn: true, transcript: "So the plan for this quarter is set.",
    ...
    func testTurnsAreKeyedByTurnOrderPerStream() {
        ...
        transcriber.handleServerMessage(
            makeTurn(order: 0, endOfTurn: false, transcript: "first turn"), for: .you)
        transcriber.handleServerMessage(
            makeTurn(order: 1, endOfTurn: false, transcript: "second turn"), for: .you)
        // Same turn_order on the other stream is a distinct segment.
        transcriber.handleServerMessage(
            makeTurn(order: 0, endOfTurn: false, transcript: "their first turn"), for: .others)
        ...
        transcriber.handleServerMessage(
            makeTurn(order: 0, endOfTurn: false, transcript: "first turn updated"), for: .you)
        XCTAssertEqual(
            transcriber.segments.map(\.text), ["first turn updated", "second turn", "their first turn"])
    }
    ...
    func testTimestampsDeriveFromWordTimes() {
        ...
        transcriber.handleServerMessage(
            makeTurn(
                order: 0, endOfTurn: true, transcript: "Timed words here.",
    ...
    func testUnknownSpeakerLabelMapsToNil() {
        ...
        transcriber.handleServerMessage(
            makeTurn(
                order: 0, endOfTurn: true, transcript: "Unattributed speech here.", speakerLabel: "UNKNOWN"),
            for: .others
        )
        XCTAssertEqual(transcriber.segments.count, 1)
    ...
    func testEchoDuplicateOnMicIsSuppressedKeepingOthersCopy() {
        ...
        transcriber.handleServerMessage(
            makeTurn(
                order: 0, endOfTurn: true, transcript: phrase,
    ...
    func testEchoDuplicateOnSystemStreamEvictsEarlierMicCopy() {
        ...
        XCTAssertEqual(transcriber.segments.count, 1)
        XCTAssertEqual(
            transcriber.segments.first?.speaker, .others, ".others copy replaces the earlier .you copy")
    }
    ...
    func testDistinctSentencesOnBothStreamsAreKept() {
        ...
        transcriber.handleServerMessage(
            makeTurn(
                order: 0, endOfTurn: true, transcript: "Let me share my screen with everyone now.",
                words: [makeWord("Let", start: 1000, end: 1100)]),
            for: .you
        )
        transcriber.handleServerMessage(
            makeTurn(
                order: 0, endOfTurn: true, transcript: "The quarterly numbers look very strong indeed.",
