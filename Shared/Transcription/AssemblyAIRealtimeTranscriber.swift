...
final class AssemblyAIRealtimeTranscriber: ObservableObject {
    ...
    /// Folds turns into `segments`; the coordinator republishes `segments` after each mutation.
    private var accumulator = TranscriptTurnAccumulator()
    ...
    func handleServerMessage(_ message: AssemblyAIServerMessage, for speaker: TranscriptSegment.Speaker) {
        ...
        case .begin(let begin):
            accumulator.recordConnect(speaker: speaker, elapsed: currentElapsed())
        ...
        case .turn(let turn):
            let confirmed = accumulator.handleTurn(turn, speaker: speaker, currentElapsed: currentElapsed())
            segments = accumulator.segments
            if let confirmed { onSegmentConfirmed?(confirmed) }

        case .speakerRevision(let revision):
            accumulator.applySpeakerRevision(revision, speaker: speaker)
            segments = accumulator.segments
