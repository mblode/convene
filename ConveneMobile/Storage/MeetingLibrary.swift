...
///
/// Every save already writes a structured `<stem>.transcript.json` next to nothing — it goes to
/// the app-internal transcripts directory, deliberately kept out of the user's notes folder (see
...
/// folder or an iCloud Drive vault the app can only reach while security-scoped.
@MainActor
final class MeetingLibrary: ObservableObject {
    ...
    func reload() {
        ...
        // The directory is created by the first save, so its absence means an empty library
    ...
    private func remove(_ url: URL, using fileManager: FileManager) {
        ...
        do {
            ...
            logError("MeetingLibrary: delete failed for \(url.lastPathComponent): \(error.localizedDescription)")
        }
    }
}
